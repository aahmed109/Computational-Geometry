...
public class convexHull {
    ...
    private static void discardWriter(ArrayList arrayList){
        try {
            BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter("src/1305074_Discard.txt"));
    ...
    private static void fileWriter(Stack stack){
        try {
            BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter("src/1305074_Result.txt"));
    ...
    private static nodeArray fileReader(String filename) throws IOException {
        ...
        try {
            ...
            if(size < 3 && size > 0){
                PrintWriter writer = new PrintWriter("src/1305074_Discard.txt");
                writer.print("");
                writer.close();
    ...
    public static void main(String[] args) {
        ...
        int newPoint1 = scanner.nextInt();
        int newPoint2 = scanner.nextInt();
        nodes newNode = new nodes(newPoint1, newPoint2);
        if(nodeArray != null) {
            int maxCol = -9999;
            int minRow;
        ...
        for (Pair pair : pairs) {
            ...
            for(int k = 0; k < stack.size() - 1; k++){
                nodes nodes1 = stack.get(k);
                nodes nodes2 = stack.get(k + 1);
                int checkVal = ((nodes2.getY() - nodes1.getY()) * (newNode.getX() - nodes2.getX())) - ((newNode.getY() - nodes2.getY()) * (nodes2.getX() - nodes1.getX()));
                if(checkVal < 0){
                    out = -1;
            ...
            nodes nodes1 = stack.get(stack.size() -  1);
            nodes nodes2 = stack.get(0);
            int checkVal = ((nodes2.getY() - nodes1.getY()) * (newNode.getX() - nodes2.getX())) - ((newNode.getY() - nodes2.getY()) * (nodes2.getX() - nodes1.getX()));
            if(out == 1 || checkVal > 0){
                System.out.println("outside");
            }
            else if(checkVal == 0 || out == 0){
